...
class DCAObservation(DiscreteObservation):
    """
    For num_doors = 2, there is an 85 % of hearing the roaring coming from the DCA door.
    ...
    def __init__(self, source_of_roar):
        if source_of_roar is not None:
            super(DCAObservation, self).__init__((1, 0)[source_of_roar[0]])
        else:
            super(DCAObservation, self).__init__(-1)
        self.source_of_roar = source_of_roar
#
#    def copy(self):
#        return DCAObservation(self.source_of_roar)
#
