...
from .dca_action import ActionType
import numpy as np


class DCAData(HistoricalData):
    """
    Used to store the probabilities that the DCA is behind a certain door.
    ...
    def __init__(self, model):
        self.model = model
        self.listen_count = 0
        ''' Initially there is an equal probability of the DCA being in either door'''
        self.door_probabilities = [0.5, 0.5]
        self.legal_actions = self.generate_legal_actions

#    def copy(self):
#        dat = DCAData(self.model)
