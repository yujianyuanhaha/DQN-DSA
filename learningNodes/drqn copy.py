...
#  E_N and T_N can be seen as an approximated Function
    ...
    def flatInputS(self, In, t):
        ...
        if length ==1:
            out.append(In)
            idx = np.argwhere(self.memory[:,self.n_features+2] == t-1)  ################# 12 -> 10, hardcore stuff 
            # 10 for the position of timeframe   #[s, a, r, time, s_]
            idx = int(idx[0,0])
...
# because memory's size is fixed, so the new sample will replace the oldest one.
    ...
    # epsilon greedy algorithm
        ...
        temp_observation = observationRL1[np.newaxis, :]

#        temp_observation = self.observation.copy()
#        temp_observation = observationRL1
