...
#        bias_initializer = b_initializer
        ...
        # ------------------ build evaluate_net ------------------
        with tf.variable_scope('eval_net'+str(int(random.random()*100)), reuse=tf.AUTO_REUSE):   #, reuse=tf.AUTO_REUSE
        ...
        # ------------------ build target_net ------------------
        with tf.variable_scope('target_net'+str(int(random.random()*100)), reuse=tf.AUTO_REUSE):  #,reuse=tf.AUTO_REUSE
        ...
        with tf.variable_scope('q_eval'+str(int(random.random()*100))):
            a_indices = tf.stack([tf.range(tf.shape(self.a)[0], dtype=tf.int32), self.a], axis=1)
            self.q_eval_wrt_a = tf.gather_nd(params=self.q_eval, indices=a_indices)    # shape=(None, )
        with tf.variable_scope('loss'+str(int(random.random()*100))):
